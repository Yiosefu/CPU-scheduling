...
class SJF:
    ...
    def update_table(self):
        ...
        run_button = tk.Button(self.input_frame, text="Run SJF", width=15, font=('Helvetica', 14), anchor="center", bg="#F7E59D", fg="black", command=self.Calculate_SJF)
    ...
    def sjf(self, job_list):
        ...
        while index < len(job_list) or job_queue:
            ...
            while index < len(job_list) and job_list[index][0] <= time:
                heapq.heappush(job_queue, (job_list[index][1], job_list[index][0], job_list[index][2]))
                index += 1

            if job_queue:
                # Pop the job with the shortest burst time from the heap
    ...
    def Calculate_SJF(self):
        ...
        gantt, completed, timeline = self.sjf(process_list)
