...
class NPP:
    ...
    def update_table(self):
        ...
        run_button = tk.Button(self.input_frame, text="Run NPP", width=15, font=('Helvetica', 14), anchor="center", bg="#F7E59D", fg="black", command=self.Calculate_NPP)
        run_button.grid(row=num_jobs+2, column = 0, columnspan=5)

    #dito icocompute tapos isosort yung algorithm
    def npp(self, job_list):
        time = 0
        gantt = []
        timeline = []
        completed = {}
        job_list.sort(key=lambda x: x[0])  # Sort jobs by arrival time
        job_queue = []

        index = 0  # Pointer for job_list

        while index < len(job_list) or job_queue:
            # Add all jobs that have arrived by the current time to the heap
            while index < len(job_list) and job_list[index][0] <= time:
                heapq.heappush(job_queue, (job_list[index][2], job_list[index][0], job_list[index][1], job_list[index][3]))
                index += 1

            if job_queue:
                # Pop job with the lowest priority, and if tied, the earliest arrival time
                ...
                tat = ct - arrival_time
                wt = tat - burst_time
                gantt.append((job_name, arrival_time, ct))
                timeline.append((job_name,arrival_time, start_time, ct))
                completed[job_name] = [arrival_time, burst_time, ct, tat, wt]
            else:
    ...
    def Calculate_NPP(self):
        try:
            ...
            for arrival_entry, burst_entry, priority_entry in self.process_list_list:
                ...
                job_name = f"p{len(process_list) + 1}"
                process_list.append([arrival_time, burst_time, priority, job_name])
        except ValueError:
            messagebox.showerror("Error", "Please enter valid numbers for arrival and burst times.")
            return

        gantt, completed, timeline = self.npp(process_list)
        ...
        self.gantt_text.insert(tk.END, "\n")
        for task, arrival_time, start_time, ct in timeline:
            self.gantt_text.insert(tk.END, f"{task}({ct}){' ' * 4}||")
        self.gantt_text.insert(tk.END, "\n")
        ...
        for pid, values in completed.items():
            self.completion_text.insert(END, f"{' ' * 6}{pid}{' ' * 13}{values[0]}{' ' * 16}{values[1]}{' ' * 21}{values[2]}{' ' * 24}{values[3]}{' ' * 18}{values[4]}{' ' * 10}\n")
